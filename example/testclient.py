...
def bad_server_public_key():
	...
	global params
	client = SecurestClient(client_certificate_id=params['client_certificate_id'],
			client_public_key=params['client_public_key'],
...
def tampered_payload():
	...
	data = {'name': 'ashish', 'last': 'dubey'}
	(url, headers, content) = client.create_request('http://localhost:8000/test/', headers={},
		data=json.dumps(data))

	(headers, res) = client.send((url, headers, 'tampered'))
